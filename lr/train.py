...
# Hyperparameters
random_seed = 1
learning_rate = 0.001  # todo : change with step
num_epochs = 100
batch_size = 256
dropout_prob = 0.7

# Architecture
num_features = 632   # todo : compute
...
if os.path.exists('./model_all/model_{}.pkl'.format(best_epoch)):
    print('loading model from ./model_all/model_{}.pkl ...'.format(best_epoch))
    model.load_state_dict(torch.load('./model_all/model_{}.pkl'.format(best_epoch)))
...
for epoch in range(num_epochs):
    ...
    torch.save(model.state_dict(), './model_all/model_{}.pkl'.format(epoch))  
...
model.load_state_dict(torch.load('./model_all/model_{}.pkl'.format(num_epochs-1)))
